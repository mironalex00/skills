...
```
You're testing that the mock returns what you configured it to return. Tautology. Mocks should verify *interactions* (was the function called with the right args?) or isolate *real* code from external dependencies — not be the subject of the assertion themselves.
...
2. For each, write a test that *attempts* to cover it.
