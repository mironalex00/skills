...
Twenty rules for code that is cheap to change and easy to test. Each rule ships a bad/good pair in a real language and ties back to testability — if a function resists a test, a rule is being broken somewhere. Use it while writing, reviewing, or refactoring. Cite the rule number in review, not your taste. The rules cover naming, functions, SOLID, component cohesion, error handling, testability, and state.

## The rules

### 1. Reveal intent in every name
A name answers *what*, not *how*. If you need a comment to explain the name, the name failed.
```typescript
