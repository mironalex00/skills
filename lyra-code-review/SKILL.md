...
```ts
test('function runs', () => { myFunction(); expect(true).toBe(true) })
```
...
```ts
test('uses a Map internally', () => { expect((cache as any).store).toBeInstanceOf(Map) })
```

6. **Catch-all try/catch swallowing failures** — the test passes whether the function works or throws.
```ts
test('works', () => {
  try { myFunction() } catch (e) { /* ignore */ }
